...
LOG=${1:-${OUTPUT_DIR}/console-user-channel-fault.log}
TIMEOUT=${2:-180}
"${SCRIPT_DIR}/build.sh"
timeout --signal=KILL "${TIMEOUT}" qemu-system-aarch64 \
	...
	-append "console=ttyAMA0 kvm-arm.mode=protected earlycon rdinit=/init pvmfault=1" > "${LOG}" 2>&1
rc=$?
echo "QEMU_RC=${rc}"
grep -E "PVM_USER_|Mlocked:|Kernel panic|Oops|BUG:" "${LOG}" || true
if [ "${rc}" -ne 0 ] || ! grep -q "PVM_USER_FAULT_VALIDATION_OK" "${LOG}"; then exit 1; fi
if grep -q "Kernel panic\|Oops\|BUG:" "${LOG}"; then exit 1; fi
echo "PVM_USER_CHANNEL_FAULT_OK"

...
**이 스크립트는 673커밋을 모두 올린 트리에서는 그대로 동작하지 않는다.** `arch/arm64/kvm/Kconfig` 파일 히스토리를 직접 읽은 결과, 다음 ANDROID 커밋들이 해당 심볼을 이름 변경했다(모두 T1 core KVM/hyp 경로, ANDROID: 접두, 필터 통과 대상).
...
**병합 후(673커밋 적용 후) 트리에서 원래 의도(KVM + 디버그 모드 + 스택트레이스)를 재현하려면 다음이 필요하다.**
```

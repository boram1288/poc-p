...
Linux v6.18 위에 pKVM 코어, IOMMU/SMMUv3, pvIOMMU 및 EL2 벤더 모듈을 포함한 검증 트리를
구성하고 clang과 gcc 양쪽에서 빌드한다.
...
pKVM 및 IOMMU 기능을 위해 다음 설정을 활성화한다.
...
검증 트리에는 ACK 헤더 보충, 무관한 IOMMU 벤더 훅 제거, DMA-BUF 링키지 수정과

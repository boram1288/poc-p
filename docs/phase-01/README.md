...
- 타깃 커널 v6.18과 두 pKVM 브랜치의 역할이 근거와 함께 확정됐다.
- 경로 기반 집합(658)과 실제 빌드 집합(721)이 분리 기록됐다.

## 한계

커밋 집계는 조사 시점 기준이다. 브랜치가 갱신되면 수치가 달라진다. 실제 검증 트리의
기준은 `pkvm-6.18-full` 브랜치의 이력이다.

이 Phase는 소스 선정만 다룬다. 빌드 가능 여부는 Phase 02에서 판정한다.

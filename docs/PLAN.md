...
| G-11 | AI 추론 결과 반환 | AI pVM이 추론을 완료하고 허용된 결과만 Host Application에 반환 | 10 | 미착수 |
...
성공 조건 3과 5는 QEMU 에뮬레이션 장치로 판정한다. 장치 할당 경로, 소유권 전환, DMA 격리
로직, 결과 반환 경로가 성립하는지를 확인하는 것이 판정 대상이다. 실제 USB 카메라와 NVIDIA
GPU에 대한 동작과 기밀성은 이 PoC에서 주장하지 않는다.

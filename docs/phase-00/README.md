...
E-3의 정의는 D-9에서 H-6(QEMU `virt,iommu=smmuv3`)을 채택하면서 확정했다. 실물 하드웨어는
PoC 범위에서 제외했으므로 E-3는 에뮬레이션 환경이며 장치 DMA를 포함한 실물 기밀성을
주장하지 않는다. 현재 정의는 [전체 수행 계획](../PLAN.md)의 3절을 따른다.
